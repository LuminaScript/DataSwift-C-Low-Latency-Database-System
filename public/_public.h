/****************************************************************************************/
/*   Program Name: _public.h. This program is the declaration file for common functions  */
/*   and classes of the development framework.                                          */
/****************************************************************************************/
...
// This class provides functionalities for handling log file operations.

class CLogFile
{
public:
  FILE   *m_tracefp;           // Pointer to the log file.
  char    m_filename[301];     // Name of the log file. Absolute path recommended.
  char    m_openmode[11];      // Mode for opening log file, typically "a+".
  bool    m_bEnBuffer;         // Enables OS buffering when writing logs (default: disabled).
  bool    m_bBackup;           // Enables auto-switching if log size exceeds m_MaxLogSize (default: enabled).
  long    m_MaxLogSize;        // Maximum log file size in MB (default: 100MB).
  // pthread_spinlock_t spin;  // For advanced use; beginners may ignore.

  /**
   * @brief Constructor to create a log file object.
   * 
   * @param MaxLogSize The maximum size of the log file in MB. Defaults to 100MB, minimum is 10MB.
   */
  CLogFile(const long MaxLogSize=100);

  /**
   * @brief Opens the log file with specified parameters.
   * 
   * @param filename  Name of the log file with an absolute path recommended.
   * @param openmode  Mode to open the log file, similar to fopen (default: "a+").
   * @param bBackup   Auto-switch option (true: enable, false: disable). Must be false if shared among processes.
   * @param bEnBuffer Enables file buffer mechanism (true: enable, false: disable). If enabled, writing is not immediate (default: disabled).
   * @return True if the operation is successful, false otherwise.
   */
  bool Open(const char *filename, const char *openmode=0, bool bBackup=true, bool bEnBuffer=false);

  /**
   * @brief Renames and backs up the log file if the size exceeds m_MaxLogSize.
   * 
   * @note In multi-process programs, log files must not switch. In multi-threaded programs, they can.
   * @return True if the backup is successful, false otherwise.
   */
  bool BackupLogFile();

  /**
   * @brief Writes content to the log file.
   * 
   * @param fmt Format string (variable argument), similar to printf.
   * @return True if the write is successful, false otherwise.
   * @note Write includes current time, WriteEx does not.
   */
  ...
  /**
   * @brief Destructor that calls the Close method.
   */
  ~CLogFile();
};
