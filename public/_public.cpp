...
// Adds the current process's heartbeat information into the shared memory process group.
bool CPActive::AddPInfo(const int timeout, const char *pname, CLogFile *logfile)
{
  ...
  // Create/get shared memory. The key is SHMKEYP and the size is MAXNUMP * sizeof(struct st_procinfo).
  ...
  // Process IDs are recycled. If a process exits abnormally without cleaning its heartbeat information,
  // the daemon might accidentally kill the current process when it reuses the previous process's ID.
  ...
  if (m_pos == -1)
  {
    // Find an empty position if the current process's ID is not found in the shared memory process group.
    for (int ii = 0; ii < MAXNUMP; ii++)
      if ((m_shm + ii)->pid == 0) 
      {
        m_pos = ii;
  ...
  if (m_pos == -1)
  {
    ...
    m_sem.V(); // Unlock the shared memory.

    return false;
  }

  // Store the current process's heartbeat information in the shared memory process group.
